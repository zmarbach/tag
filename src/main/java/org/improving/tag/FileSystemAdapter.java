...
public class FileSystemAdapter {
    public String saveToFile(Map<String, String> contents) throws IOException {
        ...
        try (var writer = new FileWriter(file)) {//create new FileWriter and give it the temp file
            for (var pair : contents.entrySet()){//.stream().sorted((x,y) -> x.getKey().compareTo(y.getKey())).collect(Collectors.toList())) {//have FileWriter look through all pairs of keys and values in the Hashmap (variable called contents above)
                writer.append(pair.getKey() + "|" + pair.getValue() + "\n");//for every pair in the temp file, write the key and value to the file
            }
        }
